...
# analyze -a DNN -c jdbc:sqlserver://192.168.0.63;databaseName=dnn;trustServerCertificate=false;encrypt=false -u theuser -pwd thepass --expunge-scan-sessions
# Must modify $CL_SQLCMD to reflect the proper connection strinng as shown above.  Need databaseName=
CL_SQLCMD="analyze -a ${DB_APPNAME} -c jdbc:${DB_TYPE}://${DB_HOST};databaseName=${DB_NAME};${DB_XTRA} -u ${DB_USER} -pwd ${DB_PASS} --expunge-scan-sessions"
echo ${CL_SQLCMD}
